...
def fetch_stats(selected_user, df):
    if selected_user == 'Overall':
        ...
        for message in new_df['message']:
            words.extend(message.split())
        # 3. number of media messages
        num_media_messages = new_df[new_df['message'] == '<Media omitted>/n'].shape[0]
        # 4. fetch number of links shared
        links = []
        for message in df['message']:
            links.extend(extract.find_urls(message))
...
def most_busy_users(df):
    ...
    df = round(df['user'].value_counts(normalize=True) * 100, 2).reset_index().rename(
...
def create_wordcloud(selected_user, df):
    ...
    df_wc = wc.generate(string_true_words)
    return df_wc
...
def activity_heatmap(selected_user, df):
    ...
    df_crosstab = df.pivot_table(index='day_name', columns='period', values='message', aggfunc='count').fillna(0)
